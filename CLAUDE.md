...
This is a CachyOS installation automation repository with Niri window manager configuration. It provides a reproducible setup for CachyOS CLI installations with 202 official packages (system-agnostic), hardware-specific drivers (GPU/CPU configured per system), 4 AUR packages (including dms-shell-git), and 14 flatpak applications. The setup uses the Limine bootloader with optional secure boot support. All configurations are hardware-agnostic with no hardcoded monitor settings.
...
5. **Monitor Configuration**: Add monitor configuration to `.config/niri/config.kdl` (see lines 9-32 for examples)
